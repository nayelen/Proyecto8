...
const postPlataforma = async (req, res, next) => {
  try {
    ...
    if (req.file) {
      newPlataforma.image = req.file.path;
    }
    const plataformaSaved = await newPlataforma.save()
    return res.status(200).json(plataformaSaved);
  } catch (error) {
    return res.status(400).json("Error en la solicitud")
  }
};
const putPlataforma = async (req, res, next) => {
  try {
    ...
    if (req.file) {
      newPlataforma.image = req.file.path;
      deleteFile(oldPlataforma.image);
    }
...
const deletePlataforma = async (req, res, next) => {
  try {
    ...
    deleteFile(plataformaDeleted.image)
...
module.exports = { getPlataformas, getPlataformaById, postPlataforma, putPlataforma, deletePlataforma }